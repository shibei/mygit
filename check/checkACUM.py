...
os.chdir(r"D:\check\logacum")                                     #切换目录
...
#show cpu  | include five minute /'CPU利用率'
            if 'show cpu  | include five minute' in line_list[x].strip():
                check_list['CPU利用率']=checks(line_list[x].strip(),'CPU',34)


#show version
            if 'show version' in line_list[x].strip():
                count=1
                while 'BJBJ-WLAN' not in line_list[x+count].strip():
                    check_list['上次启动时间']=checks(line_list[x+count].strip(),'System start time',26)
                    check_list['上次启动时间']=checks(line_list[x+count].strip(),'System start time',26)
                    check_list['硬件信息']=checks(line_list[x+count].strip(),'System hardware version',26)
                    check_list['设备版本']=checks(line_list[x+count].strip(),'System software version',26)
                    count=count+1
                print check_list['上次启动时间'],check_list['上次启动时间'],check_list['硬件信息']，check_list['设备版本']
...
#show ssh/'检查SSH远程登录状态'
            if 'show ssh' in line_list[x].strip():
                ...
                elif cut(line_list[x+2].strip(),7)!='ruijie':
...
#show snmp host/'检查SNMP服务器配置'
            if 'show snmp host' in line_list[x].strip():
                ...
                elif cut(line_list[x+7].strip(),2)!='10.223.16.8':
...
#show acum hot-backup/'主备关系'
            if 'show acum hot-backup' in line_list[x].strip():
                check_list['主备关系']=cut(line_list[x+5].strip(),4)

#show wlan-ac ap-config summary/'在线AP数检查(总数，在线数)'
            if 'show wlan-ac ap-config summary' in line_list[x].strip():
                check_list['在线AP数检查(总数，在线数)']=str(int(cut(line_list[x+7].strip(),3))+int(cut(line_list[x+8].strip(),3)))+\
                '/'+cut(line_list[x+7].strip(),3)
                
#show wlan-ac wlan users/'在线用户数检查'
            if 'show wlan-ac wlan users' in line_list[x].strip():
                count=1
                user_num=0
                while 'BJBJ' not in line_list[x+count].strip():
                    if 'total' in line_list[x+count].strip():
                        user_num=user_num+int(cut(line_list[x+count].strip(),2).split(':')[1])
                    count=count+1
                check_list['在线用户数检查']=str(user_num)
#                check_list['在线用户数检查']=cut(line_list[x+1].strip(),2).split(':')[1]
                
#show wlan-ac wlan-config summary/'SSID'
            if 'show wlan-ac wlan-config summary' in line_list[x].strip():
                count=6
                wlan_list=[]
                ssid_list=[]
                while not line_list[x+count].strip()=='':
...
#show ip dhcp pool/'查看用户地址池ip地址分配情况'
            if 'show ip dhcp pool' in line_list[x].strip():
                check_list['查看用户地址池ip地址分配情况']=cut(line_list[x+5].strip(),3)+r'%'
...
#结果写入excel文件
    for col in range(len(list_title)):
        #print list_title[col].decode('utf-8'),check_list[list_title[col]].decode('utf-8')
        table.write(row,col,check_list[list_title[col]].decode('utf-8'))

workbook.save('d:/check/ACUMcheck.xls')
